#!/usr/bin/env python3
"""Generate webapps.html and 33 individual webapp detail pages."""
...
INDEX_JSON = BASE_DIR / "webapp" / "_index.json"
WEBAPP_DIR = BASE_DIR / "webapp"
SOURCE_DIR = Path("D:/NEXTCLOUD/Web_app")

# Category config
CATEGORIES = [
    {"name": "AI系",          "anchor": "ai",        "emoji": "&#x1F916;"},
    {"name": "ヘルスケア",     "anchor": "health",    "emoji": "&#x1F48A;"},
    {"name": "ビジネス",       "anchor": "business",  "emoji": "&#x1F4BC;"},
    {"name": "コミュニケーション", "anchor": "comm",   "emoji": "&#x1F4AC;"},
...
def read_readme(app_dir_name):
    """Try to read README.md from the source directory."""
    base = SOURCE_DIR / app_dir_name
    # Direct README
...
def generate_feature_bullets(desc, tech):
    """Generate feature bullet points from description and tech."""
    bullets = []
    ...
    keywords = {
        ...
        "リアルタイム": "リアルタイム処理・同期機能",
        "PDF": "PDF出力・保存機能",
        "PWA": "PWAとしてスマホからも利用可能",
        "OCR": "画像からのテキスト認識（OCR）機能",
        "カメラ": "カメラを使った撮影・スキャン機能",
        "WASM": "WebAssemblyによる高速処理",
        "オフライン": "オフライン対応",
        "ドラッグ": "ドラッグ&ドロップ操作に対応",
        "多言語": "多言語対応",
        "多通貨": "多通貨対応",
    ...
    # Always add responsive design bullet
    if len(bullets) < 2:
        bullets.append("シンプルで使いやすいインターフェース")

    if len(bullets) < 3:
        bullets.append("ブラウザだけで動作、インストール不要")

    if "Cloudflare" in (tech or ""):
        bullets.append("Cloudflareによる高速・安定した配信")
    elif len(bullets) < 4:
        bullets.append("モダンなWeb技術で構築")
...
# ─── Generate webapps.html ───────────────────────────────────────────────────

def generate_webapps_html(apps):
    # Group by category
    cat_groups = {}
    ...
    <section id="{cat["anchor"]}" class="category-section">
      ...
      <p class="category-count">{count} 件のWebアプリ</p>
...
'''
        for app in items:
            ...
            footer_right = ""
            if app.get("url"):
                footer_right += f'<a href="{h(app["url"])}" class="demo-link" target="_blank" rel="noopener"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>Demo</a>\n'
            if app.get("github"):
          ...
          <div class="product-item-footer">
            <a href="webapp/{h(app["slug"])}.html" class="detail-link">詳細を見る {arrow_svg()}</a>
...
<head>
  ...
  <title>Webアプリ一覧 | 合同会社シティーリバー</title>
  <meta name="description" content="合同会社シティーリバーが開発・提供するWebアプリケーションの一覧。AI、ヘルスケア、ビジネス、ライフスタイルなど33種以上。" />
...
<body>
  ...
  <main>
    <div class="breadcrumb">
      <a href="index.html">ホーム</a> / <a href="index.html#products">プロダクト</a> / Webアプリ
    </div>
    ...
    <div class="page-header">
      ...
      <h1 class="page-title">Webアプリ一覧</h1>
      <p class="page-subtitle">ブラウザで動作するWebアプリケーション。インストール不要ですぐに利用可能。</p>
    </div>
    ...
    <div class="stats-bar">
      ...
      <div class="stat">
        <div class="stat-number">React・Svelte・Hono</div>
...
def generate_detail_page(app, readme_content=None):
    name = h(app["name"])
    desc = h(app["desc"])
    ver = h(app["ver"])
    tech = h(app.get("tech", ""))
    ...
    buttons = ""
    if url:
        buttons += f'''        <a href="{h(url)}" class="btn btn-demo" target="_blank" rel="noopener">
...
'''
    buttons += f'''        <a href="../webapps.html" class="btn btn-outline">
          {back_arrow_svg()}
          一覧に戻る
        </a>'''
...
{tech_html}
    ...
    if url:
      ...
      <h2>デモ・公開URL</h2>
      <p><a href="{h(url)}" target="_blank" rel="noopener">{h(url)}</a> からすぐにお試しいただけます。</p>'''
...
<body>
  ...
  <main>
    <div class="breadcrumb">
      <a href="../index.html">ホーム</a> / <a href="../index.html#products">プロダクト</a> / <a href="../webapps.html">Webアプリ</a> / {name}
    </div>
...
{content_html}
    ...
    <a href="../webapps.html" class="back-link">
      {back_arrow_svg()}
      Webアプリ一覧に戻る
    </a>
  </main>
...
def main():
    ...
    # 1. Generate webapps.html
    webapps_html = generate_webapps_html(apps)
    outpath = BASE_DIR / "webapps.html"
    with open(outpath, "w", encoding="utf-8") as f:
        f.write(webapps_html)
    print(f"\nGenerated: {outpath}")
    ...
    for app in apps:
        readme = read_readme(app["dir"])
        if readme:
            readmes_found.append(app["dir"])
        detail_html = generate_detail_page(app, readme)
        detail_path = WEBAPP_DIR / f'{app["slug"]}.html'
        with open(detail_path, "w", encoding="utf-8") as f:
            f.write(detail_html)
        generated.append(detail_path.name)

    print(f"\nGenerated {len(generated)} detail pages in {WEBAPP_DIR}/:")
    for name in sorted(generated):
        print(f"  {name}")

    if readmes_found:
        print(f"\nREADME.md found for {len(readmes_found)} apps:")
        for d in readmes_found:
            print(f"  {d}")
    else:
        print("\nNo README.md files found in source directories.")
